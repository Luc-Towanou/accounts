...
<head>
    <meta charset="UTF-8">
    <title>Export Mois Comptable - {{ $mois->mois }}</title>
    <style>
        body {
...
<body>
    ...
    <!-- Header -->
    <div class="header">
        <h1>{{ $mois->mois }} - {{ $mois->annee }}</h1>
        <p class="small">Export généré le {{ now()->format('d/m/Y H:i') }}</p>
        {{-- pdf/mois_comptable.blade.php --}}
        <h1 style="text-align: center;">Rapport du Mois Comptable : {{ $mois->mois }}</h1>

    </div>


{{-- ... affichage des tableaux et dépenses ... --}}
@php
        $totalDepensesMois = 0;
...
<hr>
<h2 style="margin-top: 40px; text-align:center;">Analyse Financière</h2>

<p><strong>Budget total :</strong> {{ number_format($analyse['totalBudget'], 2, ',', ' ') }} €</p>
<p><strong>Total dépensé :</strong> {{ number_format($analyse['totalDepense'], 2, ',', ' ') }} €</p>
<p><strong>Solde restant :</strong> {{ number_format($analyse['solde'], 2, ',', ' ') }} €</p>
<p><strong>Taux d'utilisation :</strong> {{ $analyse['tauxUtilisation'] }} %</p>
...
<table width="100%" border="1" cellspacing="0" cellpadding="5">
    ...
    <tbody>
        @foreach ($analyse['categories'] as $cat)
        <tr>
            <td>{{ $cat['nom'] }}</td>
            <td>{{ number_format($cat['budget'], 2, ',', ' ') }} €</td>
            <td>{{ number_format($cat['depense'], 2, ',', ' ') }} €</td>
            <td>{{ number_format($cat['ecart'], 2, ',', ' ') }} €</td>
            <td>{{ $cat['taux_depense'] }} %</td>
        </tr>
        @endforeach
    </tbody>
</table>

@if($analyse['tauxUtilisation'] > 100)
    <p style="color: red;"><strong>Attention :</strong> Vous avez dépassé votre budget global !</p>
@elseif($analyse['tauxUtilisation'] > 80)
    <p style="color: orange;"><strong>Prudence :</strong> Vous avez consommé plus de 80% de votre budget.</p>
@else
    <p style="color: green;"><strong>Bien :</strong> Votre budget est bien maîtrisé.</p>
@endif
